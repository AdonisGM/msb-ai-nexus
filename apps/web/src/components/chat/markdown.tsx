 ...
 *
 *  Memoised, and its element map built once at module level. Both matter
 *  while a reply streams: the panel re-renders on every frame of text, and a
 *  `components` object made inline is a new set of element *types* each time
 *  — React then unmounts and rebuilds every bubble already in the thread,
 *  tables and all, dozens of times a second. That was the stutter. */
export const Markdown = memo(function Markdown({
  ...
  return (
    <ReactMarkdown
      ...
      components={inverted ? INVERTED : PLAIN}
    >
      {text}
    </ReactMarkdown>
  )
...
function elements(inverted: boolean): Components {
  return {
...
const PLAIN = elements(false)
const INVERTED = elements(true)
